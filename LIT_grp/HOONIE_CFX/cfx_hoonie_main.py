...
from LIT_grp.HOONIE_CFX.ui import cfx_HOONIE_UI
from LIT_grp.HOONIE_CFX.handler import cfx_shotgun_handler, cfx_maya_handler, cfx_shader_importer
from LIT_grp.HOONIE_CFX.model import cfx_import_model, cfx_imported_model

reload(connect_sg)
reload(cfx_HOONIE_UI)
reload(cfx_shotgun_handler)
reload(cfx_maya_handler)
reload(cfx_shader_importer)
reload(cfx_import_model)
...
class HoonieMain(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.ui = cfx_HOONIE_UI.Ui_HOONIE_QWidget()
        ...
        cfx_maya_handler.set_plugin_info()

        # config model
        self.import_model = None
        self.import_proxy_model = QtCore.QSortFilterProxyModel()
        self.import_item_delegate = cfx_import_model.PubItemDelegate()
        ...
        self.imported_item_delegate = cfx_imported_model.ImportedItemDelegate()
    ...
    def open_shot_page(self):
        cfx_shotgun_handler.open_shot_url(sg=self.sg, context=self.context)
    ...
    def set_imported_model(self):
        ...
        imported_pub_items = cfx_maya_handler.get_imported_item_model()
        pub_items = cfx_shotgun_handler.get_published_files(sg=self.sg, context=self.context)
        ...
        for asset_name in row_datas:
            shaders = cfx_shotgun_handler.get_published_shd_file(sg=self.sg,
                                                             context=self.context,
                                                             asset_name=asset_name)
            row_datas[asset_name]['shader'].extend(shaders)


        self.imported_model = cfx_imported_model.PubItemTreeModel(row_datas=row_datas)
    ...
    def replace_imported_item(self, index):
        ...
        cfx_maya_handler.replace_alembic_reference(row_data=row_data)
    ...
    def assign_shader(self, index):
        ...
        cfx_shader_importer.import_shader(row_data=shader_row_data, target_geos=target_meshes)
    ...
    def select_item(self):
        ...
        for i in sel_data:
            if i.prefix != '__MAT':
                cfx_maya_handler.select_imported_item(i)
    ...
    def set_import_model(self):
        ...
        exist_file_names = cfx_maya_handler.already_exists_files()
        pub_items = cfx_shotgun_handler.get_published_files(sg=self.sg, context=self.context)

        self.import_model = cfx_import_model.PubItemTreeModel(row_datas=pub_items, exist_files=exist_file_names)
    ...
    def import_into_current_scene(self, index):
        ...
        cfx_maya_handler.import_pub_item(row_data=row_data, context=self.context)
        self.set_model()
    ...
    def import_selected_items(self):
        ...
        for item in sel_items:
            cfx_maya_handler.import_pub_item(row_data=item, context=self.context)
...
import LIT_grp.HOONIE_CFX.cfx_hoonie_main

reload(LIT_grp.HOONIE_CFX.cfx_hoonie_main) 
print(LIT_grp.HOONIE_CFX.cfx_hoonie_main.__file__)
LIT_grp.HOONIE_CFX.hoonie_main.maya_main_run()
"""